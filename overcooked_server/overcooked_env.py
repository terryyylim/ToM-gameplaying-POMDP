...
class OvercookedEnv(MapEnv):
    ...
    def initialize_world_state(self, items: Dict[str, List[Tuple]], ingredients: Dict[str, List[Tuple]]):
        ...
        for item in items:
            ...
            #         self.world_state[item].append(new_item)
                ...
                pot_idx = 1
                for i_state in items[item]:
                    new_item = Pot(pot_id=pot_idx, category='utensils', location=i_state,
                                   ingredient='', ingredient_count=defaultdict(int))
                    self.world_state[item].append(new_item)
                    pot_idx += 1
    ...
    def find_agents_best_goal(self):
        ...
        for agent in agents_possible_goals:
            if isinstance(agent, RLAgent):
                ...
                action_validity, action_type, action_task, goal_id = self._check_action_validity(
                    agent.id, agent_action_str)
                if not action_validity:
                    assigned_best_goal[agent] = [-1, {'steps': [8], 'rewards': -2}]
                else:
                    agent_rewards = REWARDS[agent_action_str]
                    if action_type == 'movement':
                        assigned_best_goal[agent] = [-1,
                                                     {'steps': [agent_action], 'rewards': agent_rewards}]
                    else:
                        # its a task_action being taken
                        assigned_best_goal[agent] = [
                            goal_id, {'steps': action_task, 'rewards': agent_rewards}]
            else:
                tasks_rewards = [agents_possible_goals[agent][task]['rewards']
                                 for task in agents_possible_goals[agent]]
                ...
                if tasks_rewards:
                    ...
                    all_best_paths = self.generate_possible_paths(
                        agent, agents_possible_goals[agent][softmax_best_goal])

                    # best_path == -1; means there's no valid permutations, use the original path
                    if all_best_paths != -1:
                        best_path = random.choice(all_best_paths)
                        best_path.append(
                            agents_possible_goals[agent][softmax_best_goal]['steps'][-1])
                    ...
                    assigned_best_goal[agent] = [softmax_best_goal,
                                                 agents_possible_goals[agent][softmax_best_goal]]
                else:
                    ...
                    if tuple(agent.location) in self.world_state['invalid_stay_cells']:
                        ...
                        assigned_best_goal[agent] = [-1,
                                                     {'steps': [random_valid_cell_move], 'rewards': -1}]
                    else:
                        assigned_best_goal[agent] = [-1, {'steps': [8], 'rewards': -2}]
    ...
    def generate_possible_paths(self, agent, best_goal):
        ...
        for permutation in all_permutations:
            ...
            for movement in range(len(permutation)):
                temp_agent_location = [sum(x) for x in zip(
                    temp_agent_location, MAP_ACTIONS[permutation[movement]])]
        ...
        def permutations_dp(
            ...
            if cur_step == 0:
                ...
                for heuristic_action in heuristic_map:
                    ...
                    new_location = [sum(x) for x in zip(
                        list(agent.location), MAP_ACTIONS[heuristic_action])]
                    ...
                    if 'DIAGONAL' in action and 'DIAGONAL' not in heuristic_action:
                        ...
                        new_location = [sum(x)
                                        for x in zip(new_location, MAP_ACTIONS[second_action])]
                ...
                for path_info in cur_valid_paths:
                    ...
                    for heuristic_action in heuristic_map:
                        ...
                        new_reward = cur_reward + heuristic_reward
                        new_location = [sum(x) for x in zip(
                            cur_location, MAP_ACTIONS[heuristic_action])]
    ...
    def _get_pos(self, agent_id):
        agent_pos = [agent.location for agent in self.world_state['agents']
                     if agent.id == agent_id][0]
    ...
    def _check_chop_validity(self, agent_id):
        ...
        if not player_object.holding:
            ...
            all_valid_chopping_boards_pos = [
                cb.location for cb in self.world_state['chopping_board'] if cb.state != 'taken']
        ...
        if not player_object.holding:
            ...
            for surrounding_cell_xy in surrounding_cells_xy:
                ...
                if surrounding_cell in all_valid_pots_pos:
                    ...
                    ingredient_name = player_object.holding.name
                    recipe_ingredient_count = self._get_recipe_ingredient_count(
                        ingredient_name)  # assumes no recipe with same ingredient in map
