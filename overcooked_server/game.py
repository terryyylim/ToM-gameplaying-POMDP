...
class Game:
    ...
    def load_data(self):
        ...
        self.CHOPPING_BOARDS = [
            chopping_board.location for chopping_board in episode['chopping_board'] if chopping_board.state != 'taken']
    ...
    def new(
        ...
        for idx in range(1, self.player_count+1):
            ...
            if idx == '1':
                self.player_1 = Player(
                    self, idx, players[idx]['coords'][1], players[idx]['coords'][0], players[idx]['holding'])
            elif idx == '2':
        ...
        for ingredient in ingredients:
            ...
            Ingredients(self, ingredient_name, ingredient_state,
                        ingredient_coords[1], ingredient_coords[0])
        ...
        ReturnStation(self, return_station['state'],
                      return_station['coords'][1], return_station['coords'][0])
    ...
    def save_results(self):
        ...
        if not self.is_simulation:
            self.info_df.to_csv(self.experiment_folder + '/experiments_' +
                                self.env.results_filename + '.csv', index=False)

            # agent_types = [agent.is_inference_agent for agent in self.env.world_state['agents']]
            video_name_ext = helpers.get_video_name_ext(
                self.env.world_state['agents'], self.TERMINATING_EPISODE, MAP)
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYUP:
                if event.key in [
                    ...
                    player_action_validity, action_type, action_task, goal_id = self.env._check_action_validity(
                        1, event.key)
                    player_object = [
                        agent for agent in self.env.world_state['agents'] if agent.id == '1'][0]
                    ...
                    if not player_action_validity:
                        ...
                        if action_type == 'movement':
                            best_goals[player_object] = [-1,
                                                         {'steps': [action_mapping], 'rewards': reward_mapping}]
                        else:
                            # its a task_action being taken
                            # goal_id = player_object._find_suitable_goal(action_mapping, action_task)
                            best_goals[player_object] = [
                                goal_id, {'steps': action_task, 'rewards': reward_mapping}]
    ...
    def update_experiment_results(self, info_df):
        agent_1_info = [(agent.location, agent.last_action)
                        for agent in self.env.world_state['agents'] if agent.id == '1'][0]
        agent_2_info = [(agent.location, agent.last_action)
                        for agent in self.env.world_state['agents'] if agent.id == '2'][0]
    ...
    def run_simulation(self, episodes: int = 500):
        ...
        if self.env.rl_trainer:
            self.env.rl_trainer.log_explicit_results(
